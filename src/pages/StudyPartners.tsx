...
export default function StudyPartners() {
  ...
  const [pAvail, setPAvail] = useState("Flexible");

  const fetchData = async () => {
    if (!user) return;
    const { data } = await supabase
      .from("study_partner_requests")
      .select("*, profiles!user_id(display_name, university, faculty)")
      .eq("is_active", true)
  ...
  const handlePost = async () => {
    ...
    // Insert new
    const { data: newReq, error } = await supabase.from("study_partner_requests").insert({
      user_id: user.id,
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 animate-fade-in custom-scroll overflow-y-auto h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black">Study Partner Finder</h1>
          <p className="text-sm text-muted-foreground mt-1">Find someone to study with for your next exam</p>
        </div>
        <Button onClick={() => setPostOpen(true)} className="bg-primary gap-2 shrink-0">
          <Plus className="h-4 w-4" /> Looking for Partner
        </Button>
      </div>
      ...
      {myRequest && (
        <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 mb-6 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-primary">Your active post</p>
            <p className="text-xs text-muted-foreground truncate">{myRequest.subject} · {myRequest.description}</p>
          </div>
          <Button variant="ghost" size="sm" onClick={deactivateRequest} className="text-destructive shrink-0 h-8 text-xs">
            Remove post
          </Button>
        </div>
      )}
        ...
        <div className="flex border border-border/40 rounded-lg overflow-hidden bg-card shrink-0">
          {AVAILABILITY_OPTS.map(opt => (
            <button
              ...
              className={`px-3 py-2 text-xs font-medium transition-all ${filterAvail === opt ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              {opt}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 opacity-40">
          <Users className="h-12 w-12 mx-auto mb-3 text-muted-foreground" strokeWidth={1} />
          <p className="font-bold">No partners found</p>
          <p className="text-sm text-muted-foreground text-balance">Try adjusting your filters or be the first to post a request!</p>
        </div>
      ) : (
            ...
            <div key={p.id} className="bg-card border border-border/40 rounded-2xl p-5 hover:border-primary/20 transition-all space-y-3 flex flex-col">
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-sm truncate">{p.profiles?.display_name}</p>
                    <p className="text-[11px] text-muted-foreground truncate">{p.profiles?.university || "University"}</p>
                  </div>
                </div>
              ...
              {p.description && (
                <p className="text-sm text-muted-foreground leading-relaxed flex-1 line-clamp-3">{p.description}</p>
              )}
              ...
              <div className="flex gap-2 pt-2 mt-auto">
                <Button size="sm" className="flex-1 h-8 text-xs bg-primary hover:bg-primary/90" onClick={() => navigate("/messages", { state: { userId: p.user_id } })}>
                  <MessageSquare className="h-3.5 w-3.5 mr-1.5" /> Message
                </Button>
                <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => navigate("/study-groups")}>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input value={pSubject} onChange={e => setPSubject(e.target.value)} placeholder="e.g. Calculus II" />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea 
                ...
                placeholder="e.g. Preparing for the final exam, need someone to explain concepts and do practice problems together"
