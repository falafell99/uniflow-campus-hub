...
import { MessageSquare, Clock, User, ArrowUp, ChevronDown, ChevronUp, Send, Plus, Loader2, Zap, Search } from "lucide-react";
...
type Reply = { id: number; thread_id: number; author: string; content: string; created_at: string };
type Thread = { id: number; category: string; title: string; author: string; author_id: string; content: string; tags: string[]; upvotes: number; pinned: boolean; created_at: string; replies?: Reply[]; reply_count?: number };
...
const demoThreads: Thread[] = [
  ...
  { id: 2, category: "technical", title: "Help with BFS vs DFS — when to use which?", author: "Eszter N.", author_id: "", content: "I'm confused about when to use BFS vs DFS for graph traversal problems.", tags: ["Algorithms", "Help"], upvotes: 21, pinned: false, created_at: new Date(Date.now() - 18000000).toISOString(), replies: [], reply_count: 0 },
];
...
function ThreadCard({ t, onUpvote, upvotedIds }: { t: Thread; onUpvote: (id: number) => void; upvotedIds: number[] }) {
  ...
  const loadReplies = async () => {
    if (loadingReplies) return; setLoadingReplies(true);
    const { data } = await supabase.from("forum_replies").select("*").eq("thread_id", t.id).order("created_at", { ascending: true });
    const loaded = (data as Reply[]) || []; setReplies(loaded); setReplyCount(loaded.length); setLoadingReplies(false);
  };
  ...
  const postReply = async () => {
    ...
    setPosting(false);
    if (error) { toast({ title: "Failed to post reply", variant: "destructive" }); return; }
    setReplies(prev => [...prev, data as Reply]); setReplyCount(c => c + 1); setReplyText("");
  };
    ...
    <div className={`glass-card p-4 space-y-2 transition-all duration-300 ${t.pinned ? "border-primary/30" : ""}`}>
      <div className="flex items-start gap-3 cursor-pointer" onClick={toggle}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1.5 mt-2 flex-wrap">{t.tags.map(tag => <Badge key={tag} variant="outline" className="text-[10px]">{tag}</Badge>)}</div>
        </div>
        {expanded ? <ChevronUp className="h-4 w-4 text-muted-foreground shrink-0" /> : <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />}
      </div>
      {expanded && (
        <div className="ml-6 mt-3 pt-3 border-t border-border/30 space-y-3">
          {loadingReplies ? (
...
function NewThreadDialog({ open, onClose, onCreate }: { open: boolean; onClose: () => void; onCreate: (t: Thread) => void }) {
  const { user } = useAuth();
  const [title, setTitle] = useState(""); const [content, setContent] = useState(""); const [category, setCategory] = useState("general"); const [selectedTags, setSelectedTags] = useState<string[]>([]); const [posting, setPosting] = useState(false);
  ...
  const submit = async () => {
    ...
    toast({ title: "Thread created! 🎉" }); onCreate({ ...(data as Thread), reply_count: 0 });
    setTitle(""); setContent(""); setCategory("general"); setSelectedTags([]); onClose();
  };
  return (
...
export default function ForumsTab() {
  const [threads, setThreads] = useState<Thread[]>([]); const [loading, setLoading] = useState(true); const [newThreadOpen, setNewThreadOpen] = useState(false); const [isLive, setIsLive] = useState(false); const [search, setSearch] = useState("");
  const channelRef = useRef<ReturnType<typeof supabase.channel> | null>(null);
  const [upvotedIds, setUpvotedIds] = useState<number[]>(() => { try { const s = localStorage.getItem("uniflow-upvoted-threads"); if (s) return JSON.parse(s); } catch (_) {} return []; });
  useEffect(() => { localStorage.setItem("uniflow-upvoted-threads", JSON.stringify(upvotedIds)); }, [upvotedIds]);

  const loadThreads = async () => {
    const { data, error } = await supabase.from("forum_posts").select("*, forum_replies(count)").order("pinned", { ascending: false }).order("created_at", { ascending: false });
    if (!error && data && data.length > 0) {
      setThreads(data.map(row => ({ ...row, reply_count: (row.forum_replies as unknown as { count: number }[])?.[0]?.count ?? 0 })) as Thread[]);
    } else { setThreads(demoThreads); }
    setLoading(false);
  };

  useEffect(() => {
    loadThreads();
    channelRef.current = supabase.channel("forum-posts-live-community")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "forum_posts" }, loadThreads)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "forum_posts" }, payload => setThreads(prev => prev.map(t => t.id === (payload.new as Thread).id ? { ...t, ...payload.new as Thread } : t)))
  ...
  const filterThreads = (list: Thread[]) => !search.trim() ? list : list.filter(t => t.title.toLowerCase().includes(search.toLowerCase()) || t.content.toLowerCase().includes(search.toLowerCase()) || t.tags.some(tag => tag.toLowerCase().includes(search.toLowerCase())) || t.author.toLowerCase().includes(search.toLowerCase()));
  ...
  const renderList = (items: Thread[]) => {
    if (loading) return Array.from({ length: 3 }).map((_, i) => <div key={i} className="glass-card p-4 space-y-2"><Skeleton className="h-4 w-3/4" /><Skeleton className="h-3 w-full" /><Skeleton className="h-3 w-1/2" /></div>);
    if (items.length === 0) return <div className="glass-subtle rounded-xl p-12 text-center"><p className="text-4xl mb-3">{search ? "🔍" : "💬"}</p><p className="font-medium">{search ? "No threads match your search" : "No threads yet"}</p><p className="text-sm text-muted-foreground mt-1">{search ? "Try different keywords" : "Start the conversation!"}</p>{!search && <Button className="mt-4 gap-2" size="sm" onClick={() => setNewThreadOpen(true)}><Plus className="h-4 w-4" />New Thread</Button>}</div>;
  ...
  return (
    <div className="p-6 space-y-5 max-w-4xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs gap-1">{isLive ? <><Zap className="h-3 w-3 text-success" />Live</> : <><span className="h-1.5 w-1.5 rounded-full bg-muted-foreground inline-block" />Connecting</>}</Badge>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Search threads..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" /></div>
          <Button className="gap-2" onClick={() => setNewThreadOpen(true)}><Plus className="h-4 w-4" />New Thread</Button>
        </div>
      </div>
      <Tabs defaultValue="all">
        <TabsList><TabsTrigger value="all">📋 All ({allFiltered.length})</TabsTrigger><TabsTrigger value="general">💬 General ({byCategory("general").length})</TabsTrigger><TabsTrigger value="technical">⚙️ Technical ({byCategory("technical").length})</TabsTrigger><TabsTrigger value="career">💼 Career ({byCategory("career").length})</TabsTrigger></TabsList>
        <TabsContent value="all" className="mt-4 space-y-3">{renderList(allFiltered)}</TabsContent>
        {(["general", "technical", "career"] as const).map(cat => <TabsContent key={cat} value={cat} className="mt-4 space-y-3">{renderList(byCategory(cat))}</TabsContent>)}
      </Tabs>
      <NewThreadDialog open={newThreadOpen} onClose={() => setNewThreadOpen(false)} onCreate={t => setThreads(prev => [t, ...prev.filter(x => !x.pinned)])} />
    </div>
  );
}
