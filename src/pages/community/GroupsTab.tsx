...
import { Plus, Search, Users, Send, FileText, Download, Loader2, LogOut, BookOpen } from "lucide-react";
...
export default function GroupsTab() {
  ...
  const handleCreate = async () => {
    ...
    toast.success("Group created!"); setCName(""); setCSubject(""); setCDesc(""); setCreateOpen(false); fetchGroups();
  };
  const handleJoin = async (group: Group) => {
    ...
    publishToFeed("joined_group", group.id, group.name, group.subject);
    toast.success("Joined group!"); fetchGroups();
  };
  const handleLeave = async () => {
    ...
    toast.success("Left group"); setSelectedGroup(null); fetchGroups();
  };
