...
type Question = {
  id: string; user_id: string; title: string; body: string | null; subject: string | null; tags: string[];
  answer_count: number; is_resolved: boolean; created_at: string; profiles: { display_name: string } | null;
};

type Answer = {
  id: string; question_id: string; user_id: string; content: string; is_accepted: boolean; upvotes: number;
...
export default function QandA() {
  ...
  const handlePostQuestion = async () => {
    ...
    const { data: q, error } = await supabase.from("qa_questions").insert({
      user_id: user.id, title: qTitle.trim(), body: qBody.trim() || null, 
      subject: qSubject.trim() || null, tags: tagsArray
    }).select().single();
    ...
    setAskOpen(false); setQTitle(""); setQBody(""); setQSubject(""); setQTags("");
    fetchQuestions();
  };
  ...
  const handleSubmitAnswer = async () => {
    ...
    const { error } = await supabase.from("qa_answers").insert({
      question_id: selectedQuestion.id, user_id: user.id, content: newAnswer.trim()
    });
  ...
  const acceptAnswer = async (answerId: string) => {
    ...
    await markResolved(selectedQuestion.id);
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 animate-fade-in custom-scroll overflow-y-auto h-[calc(100vh-4rem)]">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black">Q&A</h1>
          <p className="text-sm text-muted-foreground mt-1">Ask questions, help others, learn together</p>
        </div>
        <Button onClick={() => setAskOpen(true)} className="bg-primary gap-2 shrink-0">
          <HelpCircle className="h-4 w-4" /> Ask Question
        </Button>
      </div>
      ...
      {!selectedQuestion ? (
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <div className="flex border border-border/40 rounded-lg overflow-hidden bg-card shrink-0">
              {(["Latest", "Unanswered", "Resolved"] as const).map(f => (
                <button
                  ...
                  className={`px-3 py-2 text-xs font-medium transition-all ${filter === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {f}
                </button>
              ))}
            ...
            <div className="relative flex-1">
              ...
              <Input placeholder="Search subject or title..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 bg-card border-border/40" />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 opacity-40">
              <HelpCircle className="h-12 w-12 mx-auto mb-3 text-muted-foreground" strokeWidth={1} />
              <p className="font-bold">No questions found</p>
              <p className="text-sm text-muted-foreground">Adjust your filters or ask a new question!</p>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <Button variant="ghost" size="sm" onClick={() => setSelectedQuestion(null)} className="-ml-2 mb-2 text-muted-foreground hover:text-foreground">
          ...
          <div className="bg-card border border-border/40 rounded-2xl p-6 space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              {selectedQuestion.user_id === user?.id && !selectedQuestion.is_resolved && (
                <Button size="sm" variant="outline" className="text-success border-success/30 hover:bg-success/10 shrink-0" onClick={() => markResolved(selectedQuestion.id)}>
                  Mark Resolved
                </Button>
              )}
            </div>
              ...
              <div key={answer.id} className={`bg-card border rounded-2xl p-5 space-y-3 ${answer.is_accepted ? "border-success/30 bg-success/5" : "border-border/40"}`}>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border/10">
                  <div className="flex items-center gap-3">
                    <button onClick={() => handleUpvote(answer.id)} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors bg-muted/20 px-2.5 py-1.5 rounded-lg">
          ...
          {!selectedQuestion.is_resolved && (
            <div className="bg-card border border-border/40 rounded-2xl p-5 space-y-3 shadow-lg shadow-background/50">
              ...
              <div className="flex justify-end">
                <Button onClick={handleSubmitAnswer} disabled={!newAnswer.trim()} className="bg-primary px-6">Post Answer</Button>
              </div>
            </div>
          )}
        </div>
      ...
      <Dialog open={askOpen} onOpenChange={setAskOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-1.5"><label className="text-sm font-medium">Details</label><Textarea value={qBody} onChange={e => setQBody(e.target.value)} placeholder="Add more context, show your attempt..." className="resize-none min-h-[100px]" /></div>
